  ...
  data () {
    return {
      ...
      arrayList: [
        ...
        { label: 'Array.from()', describe: ' 从一个类似数组或可迭代对象创建一个新的，浅拷贝的数组实例' },
        { label: 'Array.isArray()', describe: '用于确定传递的值是否是一个 Array。' },
        { label: 'Array.of()', describe: '创建一个具有可变数量参数的新数组实例，而不考虑参数的数量或类型。Array.of(1, 2, 3);' },
        { label: 'Array.prototype.concat()', describe: '用于合并两个或多个数组。此方法不会更改现有数组，而是返回一个新数组。const array3 = array1.concat(array2);' },
        { label: 'Array.prototype.copyWithin()', describe: '浅复制数组的一部分到同一数组中的另一个位置，并返回它，不会改变原数组的长度。array1.copyWithin(0, 3, 4)' },
        { label: 'Array.prototype.entries()', describe: '返回一个新的Array Iterator对象，该对象包含数组中每个索引的键/值对。const iterator1 = array1.entries()' },
        { label: 'Array.prototype.every()', describe: '测试一个数组内的所有元素是否都能通过某个指定函数的测试。它返回一个布尔array1.every(函数表达式), 注意：若收到一个空数组，此方法在一切情况下都会返回 true。' },
        { label: 'Array.prototype.fill()', describe: '用一个固定值填充一个数组中从起始索引到终止索引内的全部元素。不包括终止索引。arr.fill(value[, start[, end]])' },
        { label: 'Array.prototype.filter()', describe: '创建一个新数组, 其包含通过所提供函数实现的测试的所有元素。' },
        { label: 'Array.prototype.find()', describe: '返回数组中满足提供的测试函数的第一个元素的值。否则返回 undefined。' },
        { label: 'Array.prototype.findIndex()', describe: '返回数组中满足提供的测试函数的第一个元素的索引。若没有找到对应元素则返回-1' },
        { label: 'Array.prototype.flat()', describe: '会按照一个可指定的深度递归遍历数组，并将所有元素与遍历到的子数组中的元素合并为一个新数组返回。' },
        { label: 'Array.prototype.flatMap()', describe: '首先使用映射函数映射每个元素，然后将结果压缩成一个新数组。它与 map 连着深度值为1的 flat 几乎相同，但 flatMap 通常在合并成一种方法的效率稍微高一些。' },
        { label: 'Array.prototype.forEach()', describe: '对数组的每个元素执行一次给定的函数。' },
        { label: 'Array.prototype.includes()', describe: '方法用来判断一个数组是否包含一个指定的值，根据情况，如果包含则返回 true，否则返回false。' },
        { label: 'Array.prototype.indexOf()', describe: '返回在数组中可以找到一个给定元素的第一个索引，如果不存在，则返回-1' },
        { label: 'Array.prototype.join()', describe: '将一个数组（或一个类数组对象）的所有元素连接成一个字符串并返回这个字符串。如果数组只有一个项目，那么将返回该项目而不使用分隔符。' },
        ...
        { label: 'Array.prototype.reduce()', describe: '对数组中的每个元素执行一个由您提供的reducer函数(升序执行)，将其结果汇总为单个返回值。' },
        ...
        { label: 'Array.prototype.slice()', describe: '返回一个新的数组对象，这一对象是一个由 begin 和 end 决定的原数组的浅拷贝（包括 begin，不包括end）。原始数组不会被改变。' },
        { label: 'Array.prototype.some()', describe: '测试数组中是不是至少有1个元素通过了被提供的函数测试。它返回的是一个Boolean类型的值。' },
        { label: 'Array.prototype.sort()', describe: '用原地算法对数组的元素进行排序，并返回数组。默认排序顺序是在将元素转换为字符串，然后比较它们的UTF-16代码单元值序列时构建的' },
        { label: 'Array.prototype.splice()', describe: '通过删除或替换现有元素或者原地添加新的元素来修改数组,并以数组形式返回被修改的内容。此方法会改变原数组。' },
